...
def get_weighted_box(boxes, conf_type='avg'):
    ...
    for b in boxes:
        if conf_type == 'Bayes':
            if is_fiest_box:
                fenzi_conf_bayes = b[1]
                fenmu_conf_bayes = 1
                is_fiest_box = False
            else:
                last_fenzi_conf_bayes = conf
                fenzi_conf_bayes *= b[1]
                fenmu_conf_bayes = fenzi_conf_bayes + (1-last_fenzi_conf_bayes) * (1-b[1])
            conf = fenzi_conf_bayes / fenmu_conf_bayes
            bayes_sum_conf += conf
            box[4:] += (conf * b[4:])
        else:
            box[4:] += (b[1] * b[4:])
...
def weighted_boxes_fusion(
    ...
    if conf_type not in ['avg', 'max', 'box_and_model_avg', 'absent_model_aware_avg', 'Bayes']:
